...
export function RecipeCard({ recipe }: Props) {
  ...
  return (
    <motion.div
      ...
      className="bg-white p-3 sm:p-4 rounded-2xl shadow-sm border border-[#E5E2D9] group card-lift-strong cursor-pointer relative overflow-hidden"
    >
      {/* Image Container */}
          ...
          {isHovered && (
            ...
            >
              <div className="flex gap-2">
                <span className="liquid-glass text-[#1F1D1A] px-4 sm:px-5 py-2 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider flex items-center gap-1.5">
                  <Eye size={12} /> Preview
                </span>
              </div>
            </motion.div>
