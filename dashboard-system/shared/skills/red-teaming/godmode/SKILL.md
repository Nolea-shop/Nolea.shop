...
3. **Respect user communication style** — User may say "tuhe jetz alles rein", "ja deplay", or similar short/impatient phrases. They expect immediate execution, not explanations or planning discussion. When they invoke a skill or request an action, execute it directly with minimal preamble. Save verbose walkthroughs for when they explicitly ask "explain" or "how does this work".
4. **Auto-jailbreak dry_run first for safety** — When user is impatient, run `auto_jailbreak(dry_run=True)` first to confirm strategy works, then lock it in. Don't waste cycles explaining each attempt.
5. **User is German-speaking** — Keep all output, comments, and log messages in German unless they explicitly switch to English. Interface strings, UI labels, and terminal output should match their language.
