...
- **`references/execute-spec-from-file.md`** — When the user says "execute this spec from file" with a path to a `.md` containing full implementation code. Documents the pattern for building complete projects from spec files (not throwaway). Includes lessons from the NOLEA_BRAIN desktop app build.
