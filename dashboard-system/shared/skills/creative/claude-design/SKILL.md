...
- **Excessive whitespace.** Giant heroes with 120px vertical padding. Centered single-column layouts for everything. Too much space between sections. Users hate scrolling through air. Keep vertical rhythm tight: 16-24px between elements, 24-40px between sections max for content-focused apps.
...
**This user's specific preferences (Damia):**
- Keep whitespace minimal. Content-dense layouts. Max-w ~960px, compact headers (52px), tight card padding (14-16px).
- Use hand-crafted SVG icons. Never use emoji as icons. Inline SVGs in components with consistent stroke-width.
- Smooth micro-animations: fadeUp (0.35s cubic-bezier), slideDown, scaleIn. Stagger list items at 50ms intervals.
- CSS `:hover` transitions for cards (translateY -2px + shadow increase). No JS hover handlers.
- Orange (#e85d04) or similar warm accent over default blue. Food = warm colors.
- Two-column layouts for content pages (e.g., ingredients | preparation side by side).
...
- **When redesigning an existing Next.js/React repo** (not building a standalone HTML artifact): load `popular-web-designs` first, pick a template, extract its design tokens (colors, typography, spacing, radii, shadows), and apply them as CSS custom properties in the project's `globals.css`. Use inline `style={}` props or CSS modules for component-level styling. Do not create standalone HTML files — modify the existing source files in place.
